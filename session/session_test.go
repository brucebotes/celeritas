...
func TestSession_InitSession(t *testing.T) {
	...
	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("wrong kind returned tesing cookie session. Expected", reflect.ValueOf(sm).Kind(), " and got", sessKind)
	}

	if sessType != reflect.ValueOf(sm).Type() {
		t.Error("wrong type returned tesing cookie session. Expected", reflect.ValueOf(sm).Type(), " and got", sessType)
	}
}
